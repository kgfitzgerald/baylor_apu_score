...
## Data Analytics in volleyball

While this module utilizes aggregate team-level data to explore trends across all NCAA Division I teams, individual teams are increasingly collecting their own player-level and point-by-point level data to help inform their strategy. Check out [this article](https://www.avca.org/blog/the-guide-to-developing-an-analytics-strategy/) where Julie Street, a D1 volleyball coach and recruiting coordinator, discusses how their team harnesses data to inform all parts of their game, from determining play time to what defense to use against an opponent to even allocating practice time effectively. 

::: column-margin
Street, Julie. 13 July 2021. "The Guide to Developing an Analytics Strategy in Volleyball." American Volleyball Coaches Association. https://www.avca.org/blog/the-guide-to-developing-an-analytics-strategy/
:::
...
+ Which NCAA Division I women’s volleyball team had the best and worst records during the 2022-3 season? What percentage of matches did they win? (Exercise 2)
+ How many kills are typically scored per set? (Exercise 3)
+ What is the most common way of scoring a point in volleyball (kill, block, or ace)? (Exercises 4 & 5)
+ Which region has the most teams? Which conference has the most teams? How many teams are in a typical conference? (Exercises 6 & 12) 
+ If your defense tends to be strong (e.g. lots of digs), does your offense also tend to be strong (lots of kills)? (Exercise 7)
+ Which conference had the most teams with winning seasons? Which conference had the least? (Exercise 8)
+ Does the average number of attacks vary by region? (Exercise 9)
+ What's the relationship between a team's hitting percentage, their opponent's hitting percentage, and their win percentage? (Exercise 10)
+ If a team has strong hitters (that get lots of kills), are they also more likely to have strong blockers? (Exercise 11)
+ What's the relationship between assists and kills? (Exercise 13)
...
Visualizing one numeric variable shows the **distribution** of the data. We often want to comment on several things to adequately describe a distribution:

+ modality (e.g. unimodal, bimodal, multimodal, uniform)
+ skewness (e.g. right-skewed, left-skewed, symmetric)
+ center (e.g. a "typical" value as represented by the mean or median)
+ spread (e.g. the range of possible values or a summary statistic such as the IQR or standard deviation)
+ potential outliers (e.g. any unusual or extreme observations)

Don't simply report technical terms or summary stats, but rather seek to interpret what those mean in the context of the data. For example, for the visualization of `kills_per_set` above, we might describe and interpret the distribution as follows:

> Looking across all NCAA Division I team averages for the 2022 season, we see a unimodal left-skewed distribution of average number of kills per set. The vast majority of teams average between 8 and 15 kills per set, but there are a few outlier teams that average as few as 5 kills per set. A "typical" team averages about 12 kills per set, which is just under half the number of points requried to win a set (25).  
