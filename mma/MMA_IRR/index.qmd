...
authors: "Caleb Skinner; Joshua Patrick, PhD; Connor Bryson; Rodney X. Sturdivant, PhD "
format:
  html:
    theme: cosmo
    code-link: true
...
{{< video https://youtu.be/Fc5xTFmEZOM width="500" height="300">}}
...
A pdf version of this module is also available:

- Student version: [MMA_IRR_Module.pdf](MMA_IRR_Module.pdf) 
- Instructor version (with solutions): [MMA_IRR_Module_instructor.pdf](MMA_IRR_Module_instructor.pdf) 
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q1_4 <- sample(c(
   answer = "D'Amato and Lee tend to agree less often",
   "D'Amato and Lee agree the same amount",
   "D'Amato and Lee tend to agree more often"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q2_1 <- sample(c(
   answer = "Reasonable agreement (less than 30 times disagree)",
   "Excellent, they only disagree 1 time",
   "They agree very rarely (<50%, 88 times)"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q2_3 <- sample(c(
   answer = "They have slightly better agreement than D'Amato and Lee but not as strong as D'Amato and Cleary",
   "They have much worse agreement than either of the previous examples.",
   "They have much better agreement than either of the previous examples.",
   "They have agreement about as strong as D'Amato and Cleary"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q4_1 <- sample(c(
   answer = "Very good agreement (less than 15 times disagree)",
   "They agree very rarely (<50%, 71 times)",
   "Perfect agreement (0 values in the draw column)"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q4_2 <- sample(c(
   answer = "No.  Expected values are based on random selections so we would expect about 50 percent agreement",
   "Yes, it shows too much disagreement"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q4_3 <- sample(c(
   answer = "This value suggests excellent reproducibility",
   "This value suggests marginal reproducibility",
   "This value suggests good reproducibility"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q5_1 <- sample(c(
   answer = "The sample size is smaller than previous examples so we will have LESS confidence in the kappa estimate",
   "The sample size is smaller than previous examples so we will have MORE confidence in the kappa estimate",
   "The sample size is larger than previous examples so we will have MORE confidence in the kappa estimate",
   "The sample size is larger than previous examples so we will have LESS confidence in the kappa estimate"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q5_2 <- sample(c(
   answer = "They do disagree a fair amount relative to the sample size, but still near 80 percent",
   "They almost never",
   "The agree almost perfectly"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q5_3 <- sample(c(
   answer = "Expected agreement is roughly 50 percent so not surprising.",
   "Very surprising.  Expected agreement is as high as the observed."
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q5_4 <- sample(c(
   answer = "This value suggests marginal reproducibility, much worse than previous examples",
   "This value suggests good reproducibility, somewhat lower than previous examples",
   "This value suggests excellent reproducibility similar to previous examples"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q5_5 <- sample(c(
   answer = "The interval provides strong evidence of marginal reproducibility",
   "The small sample leads to a wide interval so the reproducibility has high probability of still being excellent"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q6_2 <- sample(c(
   answer = "Agreement about the fight outcome (winner) but Lee had a wider margin in scores",
   "Agreement about the fight outcome (winner) but D'Amato had a wider margin in scores",
   "Disagreement about the fight outcome with the judges picking different winners"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q6_3 <- sample(c(
   answer = "0.  There are many possible score combinations for the size of the sample.",
   "0.  The judges disagree too much.",
   "18.  Score differences of three are very likely since fights are three rounds."
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q6_4 <- sample(c(
   answer = "All less than 10.  There are many possible score combinations for the size of the sample.",
   "Large differences between judges (5 and -5).  The judges should be expected to disagree by a lot.",
   "All diagnonal values.  The judges are expected to agree often."
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q6_5 <- sample(c(
   answer = "Yes.  58 observed vs 18 percent expected.",
   "No.  58 observed vs 78 percent expected.",
   "Yes.  82 observed vs 58 percent expected.",
   "No  7 observed vs 58 percent expected."
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q7_2 <- sample(c(
   answer = "Yes.  There are only 7 times one is positive and the other negative.",
   "No.  There are many values in the off diagnonal.",
   "Yes.  There are few values in the off diagnonal."
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q7_3 <- sample(c(
   answer = "They agree more.  (71 vs 58 percent)",
   "They agree less  (58 vs 71 percent)",
   "They agree much less  (28 vs 71 percent)",
  "They agree much more  (92 vs 58 percent)"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q7_4 <- sample(c(
   answer = "Somewhat.  The highest observed values have relatively high exepected, but some high expected values are not observed often.",
   "No.  The highest observed values of 20 are 0 in the expected table.",
   "Yes. The highest expected values of 5 and above are all observed at least 10 times."
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q7_5 <- sample(c(
   answer = "Yes, the kappa value is over 0.9.",
   "No.  The observed agreement is not much better than the expected."
))
```
...
q7_6 <- sample(c(
   ...
   "We have too much variability so there is not enough evidence to conclude reproducability is excellent."
))
```
...
q7_8 <- sample(c(
   ...
   "We have evidence to support excellent reproducibility for these two judges."
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q7_9 <- sample(c(
   answer = "A large enough difference to matter when drawing inference from the confidence interval.",
   "Hardly any difference so the choice does not matter here."
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q8_1 <- sample(c(
   answer = "All but one fight",
   "About half of the fights",
   "Very rarely (less than 20 percent)"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q9_2 <- sample(c(
   answer = "Yes, Cartlidge is the only one to disagree",
   "No, they all disagree at least one time",
   "Yes, Sledge disagrees the most"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q9_3 <- sample(c(
   answer = "Just over half (53 percent)",
   "Most (about 84 percent)",
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q9_4 <- sample(c(
   answer = "Excellent, well above chance agreement (over 90 percent)",
   "Good, higher than chance agreement (84 percent)",
   "Marginal, not much better than chance agreement (65 percent)"
))
```
...
```{r, warning = FALSE, message = FALSE, eval=T, echo=F}
q9_5 <- sample(c(
   answer = "No, the interval includes the possiblity of a value that is only good",
   "Yes, we reject that kappa is equal to 0",
