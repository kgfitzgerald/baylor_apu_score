...
+ Students should be provided with the following data file (.csv) and Quarto document (.qmd) to produce visualizations and write up their answers to each exercise. Their final deliverable is to turn in an .html document produced by "Rendering" the .qmd. 
  ...
  + [Student Quarto template](student_template_ci.qmd)
...
::: column-margin
Scottie Scheffler after recieving the famed "Green Jacket"

![](scheffler_green_jacket.jpg)
...
- In golf there are a few **tours**, better thought of as leagues, that golfers regularly compete in
     ...
     - The **LIV** Tour is Saudi-backed alternative to the PGA that was formed in 2022. **LIV** is the Roman numeral for 54 and is related to the fact that **LIV** tournaments only allow 54 players and only play 54 holes, compared to the normal PGA 72 holes.
...
Single Mean confidence intervals give a range of numbers that the we can feel confident that the true population men falls between. 
...
A t-distribution table is shown below.

![](t-table.png)
Image Source: [T-Distribution Tables](https://www.tdistributiontable.com/)
...
Use the formula for creating a Confidence interval using the t-distribution and the table above to calculate the upper and lower limits of a confidence interval for the true mean of amateur scoring using the amateurs in the first round as our sample. Use a 90% confidence interval ($\alpha = .1$). 
...
A standard normal distribution (also known as a Z-distribution) is used to calculate the confidence interval for a single mean if the sample size is large enough (greater than 30) or the population standard deviation is known. The first case is common as oftentimes samples are greater than 30. The second case is rare because it is not normal to know the population standard deviation but not know the population mean. 
...
A table of Z-values is shown below:

![](z-table.png)
Image Source: [z-table.com](https://z-table.com/)
...
Use the formula for creating a Confidence interval using the standard normal distribution and the table above to calculate the upper and lower limits of a confidence interval for the true mean of pga professional scoring using the PGA pros in the first round as our sample. Use a 95% confidence interval ($\alpha = .05$).
...
There are two ways to make a decision about the null hypothesis. The first is to use a confidence interval. 

**Method 1**: Critical values, along with test statistics can be used to determine if the hypothesized population mean is within the confidence interval.

A critical value is a value that separates the rejection region from the non-rejection region. The rejection region is the area where the null hypothesis is rejected. The non-rejection region is the area where the null hypothesis is not rejected. The critical value is determined by the significance level ($\alpha$) and the degrees of freedom. The critical value is compared to the test statistic to determine if the null hypothesis should be rejected. If the critical value is within the non-rejection region, the null hypothesis is not rejected. If the critical value is within the rejection region, the null hypothesis is rejected.

Below is an example of a critical value for a 95% confidence interval (two-sided test). The critical value is 1.96. This means that if the test statistic is greater than 1.96 or less than -1.96, the null hypothesis is rejected. The blue represents the non-rejection region and the red the rejection region. Since the Test Statistic for this hypothetical example is 1.1 (less than 1.96), the null hypothesis is not rejected.
...
ggplot(new_zy, aes(x = z, y = y))+
  ...
  labs(title = "Critical Values for a 95% Confidence Interval",
...
This method ties directly to the related confidence interval.
...
The degrees of freedom for a single mean hypothesis test is $n - 1$.

For a z-test, the test statistic is calculated as follows
...
The code below tests if the mean of the vector is equal to 50. The alternative hypothesis is that the mean is not equal to 50. The confidence level is set to 95%.
...
Amateurs, who are not yet professional golfers, are generally expected to score higher than professionals. We would like to test if the mean of amateur scoring above par at Augusta National
...
What is the degrees of freedom for a single mean hypothesis test for the following above sample of Augusta National scores?
...
c. Assume that the alternate hypothesis is that the mean of the sample is greater than 72. Use the correct formula from part **b** to calculate the test statistic for the above sample manually. What is the value of the **test statistic**?
