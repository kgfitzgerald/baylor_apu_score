...
4. Confidence intervals for proportions can be used to test hypothesized values of the population proportion. If the hypothesized value is not within the confidence interval, then we have reason to believe that it is not the true population proportion.
...
The p-value of the test is returned in the output of the `binom.test()` function. For the first test, the p-value is `r round(binom.test(60, 100, p = .7)$p.value, 3)` and for the second test, the p-value is 1. Since the p-value of the first test is less than the significance level of 0.05, the null hypothesis is rejected. Since the p-value of the second test is greater than the significance level of 0.05, the null hypothesis is not rejected.
...
Use the `binom.test()` function to test the hypothesis that the proportion of greens hit in regulation by Scottie Scheffler from the fairway is 0.75. $\alpha = 0.05$.
...
The `prop.test()` function in R can be used to compare two proportions. The `prop.test()` function takes the number of successes and the total number of trials for each sample as arguments and returns a p-value for the comparison. Use the argument `correct = FALSE` in the function to get the correct results. The `prop.test()` function will list the test statistic as a X-squared value, which is the square of the Z-score. The p-value stays the same as listed in the output.
