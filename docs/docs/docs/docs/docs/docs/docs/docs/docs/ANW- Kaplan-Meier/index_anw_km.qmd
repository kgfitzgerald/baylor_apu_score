...
-   Students should be provided with the following data file (.csv) and Quarto document (.qmd) to produce visualizations and write up their answers to each exercise. Their final deliverable is to turn in an .html document produced by "Rendering" the .qmd.
    ...
    -   [data](anw_2021_stage1.csv)
    -   [Student Quarto template](student_template_km.qmd)
...
![](American_Ninja_Warrior_Logo.jpg)
...
-   How do the survival probabilities differ based on sex?
:::
...
The obstacles in American Ninja Warrior are all given names.
For example, the famed **Warped Wall** is a curved wall that competitors must run up and grab the top of. The **Salmon Ladder** is a series of rungs that competitors must move up by jumping and pulling themselves up.
...
Use the following code sum the number of competitors that failed and the number of competitors that were censored at each obstacle.
...
b. At which **obstacle** were the most competitors censored?
...
::: {.callout-note title="Part 3: Conditional Fail Probability"}

The third step is to calculate the conditional probability of surviving past each point in time. This is the $P(T \geq t_i | T \geq t_{i-1})$ value needed to calculate the Kaplan-Meier estimator and is calculated as $1 - \frac{d_i}{n_i}$.

a. Use the `ninja_summary` data frame to calculate the probability that someone fails each obstacle. Do this using the `mutate` function to create a new column called `fail_prob`. Fail probability is the number of competitors that failed divided by the number of competitors at risk. **Save this data frame as `ninja_summary`**.
...
b. What **percentage** of at-risk competitors failed the first obstacle?

c. What **percentage** of at-risk competitors failed the fifth obstacle?

d. Which obstacle had the **highest** fail probability?

e. Did **obstacle 2** or **obstacle 7** have a higher fail rate?
...
a. Use the `ninja_summary` data frame fram in conjunction with `ggplot2`'s `geom_step` and `geom_point` to plot the Kaplan-Meier estimator for the ninja competitors.

b. Comment on the plot.

c. What do you notice about where the lowest point on the plot in regard to **survival probability**? Does **survival probability** reach zero? Why or why not?
...
a. At what **time** does the lower CI endpoint imply **75%** of the competitors survive past? What time does the upper CI endpoint?

b. What is the **interpretation** of the NA values in the 75 percentile columns?

c. What is the earliest **time** (within the 95% confidence interval) the at which **50%** of the competitors could suffer a fall by?  
...
{{< video https://www.youtube.com/embed/8KIED3eCNUs >}}
...
```{r, eval=FALSE}
surv_pvalue(ninja_km_gender) #survdiff
...
b. Which **test** is the most conservative? 
...
The winner of the 2023 American Ninja Warrior
competition was Vance Walker. Walker won $1 million for his victory.
...
**Fun Fact**: Thread the Needle, the 8th obstacle on the 2021 Finals Stage 1 course, saw the highest failure rate of any obstacle.
It had not previously been used on the Finals Stage 1 courses.

![Thread the Needle](Thread_the_Needle.png)
Image Source: [Sasukepedia](https://sasukepedia.fandom.com/wiki/Thread_the_Needle)
:::
