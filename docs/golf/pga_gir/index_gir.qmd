...
::: column-margin
**TIP**: Using a hashtag in R allows you to add comments to your code (in plain English). 
:::
...
-   A **par** in golf is the number of strokes a good golfer is expected to make on a hole or course
...
Confidence Intervals for Proportions can be used for a variety of purposes, including:

1. A confidence interval is a range of values that is likely to contain the true value of a population parameter with a certain level of confidence.

2. Confidence intervals help us to quantify the precision of our estimates. The wider the confidence interval, the less precise our estimate is, the narrower the confidence interval, the more precise our estimate is.

3. Inferences can be made about the population proportion based on the sample proportion when using confidence intervals. For example, if a 90% confidence interval was used, it could be said that if the same population was sampled on numerous occasions and interval estimates were made on each occasion, approximately 90% of the intervals would contain the population parameter.

4. Confidence intervals for proportions can be used to test hypothesized values of the population proportion. If the hypothesized value is not within the confidence interval, then the null hypothesis can be rejected.

5. Confidence intervals for proportions can be used to compare two proportions. If the confidence intervals for two proportions do not overlap, then the two proportions are significantly different.
...
For a quick example if proportion of success for a problem is .6, the sample size is 100, and the test is being performed at the 95% confidence level than the confidence interval for the proportion of success can be calculated as follows:
...
This would give you the lower limit (`r lower_limit`) and upper limit (`r upper_limit`) of the confidence interval for the proportion of success.


::: {.callout-note title="Exercise 3: Manual Confidence Intervals"}

Use the numbers from row with **`cut` = "Fairway"** of the `scheffler_2023_mutated` dataset to calculate a **95% confidence interval** for the proportion of greens hit in regulation by Scottie Scheffler from the fairway. 
...
```{r}
binom.test(60, 100, conf.level = .98)
binom.test(60, 100, conf.level = .98)$conf.int
```
...
c. Based on the p-value, do you reject or fail to reject the null hypothesis?
...
The `prop.test()` function in R can be used to compare two proportions. The `prop.test()` function takes the number of successes and the total number of trials for each sample as arguments and returns a p-value for the comparison. In order for the `prop.test()` function to run a two sample z-test for equality of proportions, "correct" must be set to `FALSE`, otherwise the function will run a chi-squared test with an adjustment for continuity. Even with the "correct" argument set to `FALSE`, the `prop.test()` function will list the test statistic as a X-squared value, but it's value is the Z-score.
...
**NOTE**: `prop.test` can take the form `prop.test(c(x1, x2), c(n1, n2))` where `x1` and `x2` are the number of successes in each sample and `n1` and `n2` are the total number of trials in each sample. It can also take the form `prop.test(matrix(c(x1, x2, n1, n2), ncol = 2))` where `x1` and `x2` are the number of successes in each sample and `n1` and `n2` are the total number of trials in each sample.
...
Below is an example of how the `prop.test()` function could be used to compare two proportions: 

```{r}
prop.test(c(10, 20), c(100, 100), alternative = 't', correct = FALSE)
```

The example above compares two proportions: 10 successes out of 100 trials in the first sample and 20 successes out of 100 trials in the second sample. The `prop.test()` function returns a p-value for the comparison, which in this case is .048. 
...
Click Below to see more about rules about hitting out of the bunkers in golf:
[Bunker Rules Explained](https://www.youtube.com/watch?v=T9_gXqAE6ZY)
...
c. Is 0 included in the confidence interval for the difference in proportions?

d. Based on your answers above, what is your conclusion about the comparison of the two proportions?

e. Is there enough evidence to suggest that it is better for Scottie Scheffler to hit from the fairway than from the bunker?
...
h. Is 0 included in the confidence interval for the difference in proportions?

i. Based on your answers above, what is your conclusion about the comparison of the two proportions?

j. Is there enough evidence to reject the null hypothesis and suggest that it is better for Scottie Scheffler to hit from the "Other" cut than from bunker?
...
1. Compare Scottie Scheffler's approach shots to Rory McIlroy's approach shots in 2023. 
    ...
    b. Is Scottie Scheffler statistically better than Rory McIlroy at hitting greens in regulation from the fairway? Explain your answer.

    c. Compare the proportion of greens hit in regulation by Scottie Scheffler from the **bunker** to the proportion of greens hit in regulation by Rory McIlroy from the **bunker**. Make the alternative hypothesis that the proportion of greens hit in regulation from the bunker by Scottie Scheffler is greater than the proportion of greens hit in regulation from the bunker by Rory McIlroy. Use the **95% confidence level**. Set **correct = FALSE**.
    
    d. Is Scottie Scheffler statistically better than Rory McIlroy at hitting greens in regulation from the bunker? Explain your answer.
    
    e. Explain why it might be more difficult to find a significant difference in the proportion of greens hit in regulation from the bunker compared to the fairway.
...
3. Find the best and worst bunker players in 2023 who took more than 50 approach shots from the bunker.
    ...
    b. Test the hypothesis that the proportion of greens hit in regulation from the fairway by the best bunker player is greater than the proportion of greens hit in regulation from the fairway by the worst bunker player. Use the **99% confidence level**. Set **correct = TRUE**.
    
    c. Do you get differing results when you change the confidence level and add a continuity correction? If yes, explain why.
:::
